...
export type ApproveIssuerUpdateRequest = {
  proposalId: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: ApproveIssuerUpdateRequest = await request.json();
    const { proposalId, issuerPrivateKey } = body;

    if (!proposalId || !issuerPrivateKey) {
      return NextResponse.json(
    ...
    const tx = await contract.approveIssuerUpdate(proposalIdBigInt);
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Failed to approve issuer update" },
      { status: 500 }
    );
  }
}
