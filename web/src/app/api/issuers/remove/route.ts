...
export type RemoveIssuerRequest = {
  issuer: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: RemoveIssuerRequest = await request.json();
    const { issuer, adminPrivateKey } = body;

    if (!issuer || !adminPrivateKey) {
      return NextResponse.json(
    ...
    const tx = await contract.removeIssuer(issuer);
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Failed to remove issuer" },
      { status: 500 }
    );
  }
}
